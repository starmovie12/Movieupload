  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const processLink = async (linkData: any, idx: number) => {
        const lid = linkData.id ?? idx; // FIX: Use ?? instead of || so id:0 works correctly
        ...
        try {
          ...
          // FIX: Guard against missing/empty link
      ...
      if (taskId) {
        try {
          ...
          if (taskDoc.exists) {
            ...
            // FIX: Update existing links by matching the original URL
            // Use the link's `id` field (which is the original index) for precise matching
            const updatedLinks = existingLinks.map((existingLink: any, existingIdx: number) => {
              // Try to find a matching result by original link URL
            ...
            // FIX: Determine correct task status based on ALL links, not just this batch
            ...
            if (allDone && !anyPending) {
              // Check if ALL links failed vs some succeeded
